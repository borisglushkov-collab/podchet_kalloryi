...
- Опирайся ТОЛЬКО на переданный снимок дня и, если есть, недельный отчёт (АД, сон, шаги, вес, еда, лекарства, рабочие цели ккал)
...
def build_coach_health_prompt(
    ...
    week_block = ""
    if week_report and str(week_report).strip():
        week_block = f"Неделя (контекст):\n{str(week_report).strip()}\n\n"

    return f"""Снимок дня (источник правды):
{report}

{week_block}Полный JSON (если нужно уточнить цифры):
...
Ответь как коуч по этому дню (и неделе, если она передана): что уже ок, что поправить сегодня/завтра."""
