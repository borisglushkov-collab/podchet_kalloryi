...
- ЖЁСТКИЙ ЛИМИТ: никогда не превышай остаток ЗА ДЕНЬ (калории и каждый макрос). Если доля на приём больше дневного остатка — целишься только в дневной остаток
- Если за день осталось мало калорий (<200) или жиры/углеводы почти 0 — предлагай лёгкий белковый добор маленькой порцией, а не полноценный ужин
...
- КБЖУ каждого рецепта должно быть близко к ЦЕЛЕВОМУ остатку (обычно 70–100%, не больше 110% и не выше дневного остатка)
- Если не хватает белка — предлагай белковые блюда; если жиров — с орехами/авокадо/рыбой; если углеводов — с крупами/хлебом; если жиры/углеводы = 0 — не добавляй их сверх минимума
- Если пользователь уже что-то съел в этом приёме — предлагай ДОПОЛНЕНИЕ, а не полноценный обед с нуля
- Если передан дневник питания — ОБЯЗАТЕЛЬНО просканируй список продуктов: что уже занесено, в каких приёмах, порции и КБЖУ
- Не предлагай то же самое, что уже есть в дневнике (те же блюда/продукты), если только пользователь явно не просит повтор
- На основе дневника предлагай варианты: чем дополнить, чем заменить неудачный выбор, что убрать/урезать, если перебор по жирам/углеводам
- В top_up_summary и why_fits кратко опирайся на конкретные записи дневника (названия продуктов)
...
- Если на предыдущих приёмах норма не закрыта — недобранное КБЖУ ПЕРЕНОСИТСЯ на следующий приём, но только в пределах дневного остатка
- На последнем приёме (перекус) нужно добить ВЕСЬ остаток дня с учётом переноса (не больше дневного остатка)
...
def cap_macros_by_daily(
    meal_deficit: dict[str, float],
    daily_deficit: dict[str, float],
) -> dict[str, float]:
    """Never let a meal target exceed what remains for the whole day."""
    return {
        key: min(
            max(0.0, float(meal_deficit.get(key, 0) or 0)),
            max(0.0, float(daily_deficit.get(key, 0) or 0)),
        )
...
# Cap diary payload so prompts stay within model context.
DIARY_ENTRIES_LIMIT = 25


def format_diary_entries(entries: list[dict[str, Any]] | None) -> str:
    """Human-readable food diary block for coach / suggest prompts."""
    if not entries:
        return ""

    lines: list[str] = []
    for raw in entries[:DIARY_ENTRIES_LIMIT]:
        if not isinstance(raw, dict):
            continue
        name = str(raw.get("name") or "").strip()
        if not name:
            continue
        meal = _MEAL_RU.get(str(raw.get("meal_type", "")), str(raw.get("meal_type") or "?"))
        grams = float(raw.get("grams") or 0)
        kcal = float(raw.get("calories") or 0)
        protein = float(raw.get("protein") or 0)
        fat = float(raw.get("fat") or 0)
        carbs = float(raw.get("carbs") or 0)
        grams_part = f"{grams:.0f} г, " if grams > 0 else ""
        lines.append(
            f"- [{meal}] {name}: {grams_part}"
            f"{kcal:.0f} ккал · Б {protein:.0f} · Ж {fat:.0f} · У {carbs:.0f}"
        )
    ...
    truncated = ""
    if len(entries) > DIARY_ENTRIES_LIMIT:
        truncated = f"\n(показаны первые {DIARY_ENTRIES_LIMIT} из {len(entries)} записей)"

    return f"""
Дневник питания за сегодня (просканируй и опирайся на эти записи):
{chr(10).join(lines)}{truncated}
Задача: учти уже съеденное по названиям, не дублируй те же блюда без необходимости, \
предложи варианты дополнения/замены и добор до нормы."""
...
def build_user_prompt(
    ...
    uncapped_meal_deficit = plan["deficit"]
    rollover_in = plan["rollover_in"]
    is_last = plan["is_last"]

    daily_deficit = _deficit_macros(targets, consumed)
    meal_deficit = cap_macros_by_daily(uncapped_meal_deficit, daily_deficit)
    was_capped = any(
        meal_deficit[key] + 0.5 < uncapped_meal_deficit.get(key, 0)
        for key in _MACRO_KEYS
    )
    meal_ru = _MEAL_RU.get(meal_type, meal_type)
    ...
    if was_capped:
        capped_note = (
            "\nВАЖНО: доля на приём с переносом БОЛЬШЕ дневного остатка. "
            "Цель ниже уже урезана до дневного лимита — НЕ предлагай блюда больше этих цифр.\n"
        )
    weight_block = ""
...
Город: {city}
{profile_block}{diary_block}{last_meal_note}{capped_note}
...
{rollover_block}
Цель с учётом переноса на {meal_ru} (до урезания дневным лимитом):
...
Осталось за день (жёсткий потолок — нельзя превышать):
...
ЦЕЛЕВОЙ ОСТАТОК на {meal_ru} (min приём/день) — подбирай рецепты ТОЛЬКО под эти цифры:
...
Обязательно:
- top_up_summary — совет с привязкой к цели из профиля, дневнику и ЦЕЛЕВОМУ остатку КБЖУ на {meal_ru}
- why_fits в каждом рецепте — как блюдо помогает цели из профиля, учитывает уже съеденное и добивает норму
- reason у каждого продукта — что добивает по макросам и как связано с целью / дневником
- nutrition рецептов ≤ целевого остатка по калориям и каждому макросу
- не дублируй продукты из дневника без явной необходимости; предложи альтернативы/дополнение"""
